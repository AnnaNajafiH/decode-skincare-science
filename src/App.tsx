...
function App() {
  ...
  // Hold a preselected trend id so other views (e.g. ContentGenerator)
  // can open prefilled when a trend card calls Create Post
  const [preselectedTrendId, setPreselectedTrendId] = useState<string | null>(
    null
  );
  ...
  const renderView = () => {
    switch (currentView) {
      case "trends":
        return (
          <TrendDashboard
            onCreatePost={(trendId: string) => {
              setPreselectedTrendId(trendId);
      ...
      case "generate":
        return (
          <ContentGenerator
            preselectedTrendId={preselectedTrendId || undefined}
          />
        );
      case "review":
        return <ReviewQueue />;
